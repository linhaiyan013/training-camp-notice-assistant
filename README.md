...
同一个正式链接会先让用户选择「助理模式」或「管理员模式」，再输入对应密码进入。助理输入访问码后，可以查看、复制话术、勾选已发送；新建训练营、修改模板、删除排期、新增管理员、修改助理访问码、修改管理员主密码需要管理员密码。

当前管理员密码是 `6666`，助理访问码是 `8888`。管理员进入后，可在页面右上角「管理设置」里继续修改这两个密码。

同一台手机或同一个浏览器登录成功后，会记住上次进入的助理 / 管理员模式；除非主动退出、清理浏览器数据或密码被改掉，一般不需要重复输入。

登录后右上角会显示 `退出`，可退出当前助理 / 管理员登录模式。

助理模式不会显示管理员入口。需要切换身份时，点右上角「退出」，重新在同一个入口里选择助理或管理员即可。
