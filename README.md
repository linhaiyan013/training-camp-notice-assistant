...
今日页顶部有「到点提醒」开关。助理登录后点一次「开启提醒」，页面会记住这个设置；只要页面保持打开，到发送时间会弹浏览器通知、页面提示，并尽量触发声音 / 震动。未发送的任务会每 10 分钟再次提醒一次。

今日页也提供「导入日历」按钮，会把未来未完成的发送任务生成 `.ics` 手机日历文件。助理在手机上打开并导入后，12:00 / 19:00 等发送时间会进入手机系统日历提醒。网页无法直接批量写入 iPhone / Android 的系统闹钟 App，这是系统权限限制。

微信内置浏览器可能拦截系统通知，建议助理用手机系统浏览器打开链接，或把页面添加到手机桌面。第一版不接个人微信机器人，也不会自动发微信群。
